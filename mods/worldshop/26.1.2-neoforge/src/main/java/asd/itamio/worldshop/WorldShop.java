...
public class WorldShop {
    ...
    private void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        if (event.getEntity() instanceof ServerPlayer player) {
            ...
            economy.registerPlayer(player.getGameProfile().name(), player.getUUID());
            LOGGER.info("Registered player {} -> {}", player.getGameProfile().name(), player.getUUID());
        }
    }

    private void onServerStarting(ServerStartingEvent event) {
        LOGGER.info("World Shop recipe manager initialized");
    }
