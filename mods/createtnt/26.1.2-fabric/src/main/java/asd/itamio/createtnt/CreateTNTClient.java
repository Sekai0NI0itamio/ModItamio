...
public class CreateTNTClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        // Particle providers. 26.1.2 fabric API: ParticleFactoryRegistry was
        // renamed to ParticleProviderRegistry (same register overloads).
        ParticleProviderRegistry.getInstance().register(BetterTNTs.SHELL_EXPLOSION_SMOKE,
            sprites -> new ShellExplosionSmokeParticle.Provider(sprites));
        ParticleProviderRegistry.getInstance().register(BetterTNTs.SHELL_EXPLOSION_CLOUD,
            sprites -> new ShellExplosionCloudParticle.Provider(sprites));
        ParticleProviderRegistry.getInstance().register(BetterTNTs.SHELL_BLAST_WAVE,
