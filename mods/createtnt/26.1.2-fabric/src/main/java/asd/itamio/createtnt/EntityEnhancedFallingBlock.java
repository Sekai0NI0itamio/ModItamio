...
public class EntityEnhancedFallingBlock extends FallingBlockEntity {
    ...
    public EntityEnhancedFallingBlock(Level level, double x, double y, double z, BlockState state) {
        this(BetterTNTs.ENHANCED_FALLING_BLOCK, level);
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide()) {
            ...
            if (!this.onGround()) {
                if (this.time > 100 && (blockpos1.getY() < this.level().getMinY()
                    || blockpos1.getY() > this.level().getMaxY())) {
    ...
    private void settleBlock(BlockPos startPos) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (tryPos.getY() > this.level().getMaxY() - 1) {
                break;
            }
    ...
    protected void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput tag) {
        ...
        int stateId = tag.getIntOr("EnhancedState", -1);
        if (stateId >= 0) {
            this.syncedState = Block.stateById(stateId);
        }
    }
}
