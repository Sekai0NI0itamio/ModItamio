...
public class BlockCollapseHandler {
    ...
    public static void markSettled(World world, BlockPos pos) {
        ...
        map.put(pos, world.getTotalWorldTime() + SETTLED_GRACE_TICKS);
    }
    ...
    private int getSupportRange(World world, BlockPos pos, IBlockState state) {
        String name = state.getBlock().getRegistryName();
        if (name != null) {
            Integer override = ModConfig.supportRangeOverrides.get(name);
            if (override != null) {
                return override;
            }
        }
        ...
        return MathHelper.clamp_int(Math.round(hardness), 1, max);
    }
