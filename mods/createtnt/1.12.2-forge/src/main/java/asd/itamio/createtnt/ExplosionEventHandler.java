 ...
 *
 * <p>On entity join: vanilla {@link EntityTNTPrimed} is replaced with
...
public class ExplosionEventHandler {
    ...
    public void onExplosionStart(ExplosionEvent.Start event) {
        ...
        double x = explosion.getPosition().x;
        double y = explosion.getPosition().y;
        double z = explosion.getPosition().z;

        // Find a primed TNT at the explosion point.
        List<EntityTNTPrimed> tnts = world.getEntitiesWithinAABB(EntityTNTPrimed.class,
            new AxisAlignedBB(x - 1.0D, y - 1.0D, z - 1.0D, x + 1.0D, y + 1.0D, z + 1.0D));
        if (tnts.isEmpty()) {
            return;
        }
        ...
        // different — some pop like a normal blast, others hit up to 3×
        ...
        float powerMult = 1.0F + world.rand.nextFloat() * 2.0F;
        ...
        EnhancedExplosion enhancedExplosion = new EnhancedExplosion(world, tnts.get(0), x, y, z,
            blockPower, entityPower, false, true, particlePower);
        enhancedExplosion.doExplosionA();
        enhancedExplosion.doExplosionB(true);
        ...
        List<EntityTNTPrimed> chained = world.getEntitiesWithinAABB(EntityTNTPrimed.class,
            new AxisAlignedBB(x - chainRadius, y - chainRadius, z - chainRadius,
                x + chainRadius, y + chainRadius, z + chainRadius));
        for (EntityTNTPrimed tnt : chained) {
            tnt.setFuse(0);
        }
    }

    /**
     * Replaces vanilla {@link EntityTNTPrimed} with {@link EntityEnhancedTNTPrimed}
     * when it joins the world. This ensures all TNT — whether ignited by flint &
     * steel, redstone, arrows, or explosions — gets the water-aware fuse logic.
     *
     * <p>Only replaces on the server side. The enhanced entity is then synced
     * to clients via the Forge mod-entity spawn packet, which includes the
     * IEntityAdditionalSpawnData block state (for falling blocks) and the
     * data-manager fuse value (for TNT).</p>
     */
    @SubscribeEvent
    public void onEntityJoinWorld(EntityJoinWorldEvent event) {
        ...
        EntityTNTPrimed vanilla = (EntityTNTPrimed) event.getEntity();
        EntityEnhancedTNTPrimed enhanced = new EntityEnhancedTNTPrimed(
            event.getWorld(), vanilla.posX, vanilla.posY, vanilla.posZ, vanilla.getTntPlacedBy());
        enhanced.setFuse(vanilla.getFuse());
        enhanced.motionX = vanilla.motionX;
        enhanced.motionY = vanilla.motionY;
        enhanced.motionZ = vanilla.motionZ;

        event.setCanceled(true);
        event.getWorld().spawnEntity(enhanced);
    }
}