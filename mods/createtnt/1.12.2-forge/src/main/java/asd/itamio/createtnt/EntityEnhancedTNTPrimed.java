 ...
 *
 * <p>Water interaction rules (per user spec):</p>
 * <ul>
 ...
 * </ul>
 *
 * <p>The explosion itself is handled by {@link EnhancedExplosion} via
 * {@link ExplosionEventHandler}, which catches the vanilla
 * {@code ExplosionEvent.Start} and replaces it with the enhanced blast.</p>
 */
public class EntityEnhancedTNTPrimed extends EntityTNTPrimed {
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote && inWater && this.ticksSinceLit >= ModConfig.waterUnlightMinTicks
        ...
        if (newFuse <= 0) {
            ...
            if (!this.world.isRemote) {
                // Directly create an EnhancedExplosion instead of going
                // through world.createExplosion + ExplosionEventHandler.
                // This guarantees the explosion works in water (the event
                // handler chain can be unreliable when the TNT is
                // submerged) and avoids the vanilla explosion's water-
                // specific behaviour entirely.
                // 1.0x to 3.0x random power multiplier — each TNT varies in strength.
                float powerMult = 1.0F + this.world.rand.nextFloat() * 2.0F;
                ...
                // 1x roll = tntStrength, 3x roll = 1.5x tntStrength.
                ...
                net.minecraft.block.material.Material tntMat =
                    this.world.getBlockState(tntPos).getMaterial();
                if (tntMat == net.minecraft.block.material.Material.WATER) {
                    blockPower *= ModConfig.waterResistanceScale;
                    entityPower *= ModConfig.waterResistanceScale;
                }
                if (tntMat == net.minecraft.block.material.Material.LAVA) {
    ...
    private void unlit() {
        ...
        if (!this.world.isRemote) {
            ...
            if (this.world.mayPlace(Blocks.TNT, pos, false, EnumFacing.UP, null)) {
                this.world.setBlockState(pos, Blocks.TNT.getDefaultState(), 3);
            } else {
                this.entityDropItem(new ItemStack(Blocks.TNT), 0.0F);
            }
            this.world.playSound(null, this.posX, this.posY, this.posZ,
                SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundCategory.BLOCKS, 0.8F, 1.2F);
        }
    }
    ...
    protected void writeEntityToNBT(NBTTagCompound compound) {
        ...
        compound.setInteger("TicksSinceLit", this.ticksSinceLit);
    }

    @Override
    protected void readEntityFromNBT(NBTTagCompound compound) {
        super.readEntityFromNBT(compound);
        this.ticksSinceLit = compound.getInteger("TicksSinceLit");
    }
     ...
     *
     * @param explosion   The explosion that just occurred (its chained TNT
     *                    positions are extracted and detonated).
     * @param blockPower  Block destruction power for chain explosions.
     * @param entityPower Entity damage power for chain explosions.
     */
    ...
    private void detonateChain(EnhancedExplosion explosion, float blockPower, float entityPower) {
        ...
        detonateChainAt(this.world, this.posX, this.posY, this.posZ, this,
    ...
    private static void detonatePosition(World world, BlockPos pos, EntityEnhancedTNTPrimed cause) {
        ...
        if (mat == net.minecraft.block.material.Material.WATER) {
            chainBlockPower *= ModConfig.waterResistanceScale;
            chainEntityPower *= ModConfig.waterResistanceScale;
        } else if (mat == net.minecraft.block.material.Material.LAVA) {
     ...
     * Registered on the Forge event bus in {@link BetterTNTs#preInit}.
     */
    @SubscribeEvent
