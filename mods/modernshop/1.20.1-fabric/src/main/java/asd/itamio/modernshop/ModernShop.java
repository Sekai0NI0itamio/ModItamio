...
public class ModernShop implements ModInitializer {
    ...
    public static void saveCategoryOrder(List<ShopCategory> orderedCategories) {
        ...
        try {
            ...
            ModernShop.LOGGER.info("[CAT_ORDER] SAVED category order to {} ({} categories): {}", orderFile.getAbsolutePath(), names.size(), names);
            // Verify by re-reading immediately
            if (orderFile.exists()) {
                ModernShop.LOGGER.info("[CAT_ORDER] Verify: file exists at {}", orderFile.getAbsolutePath());
                ...
                ModernShop.LOGGER.info("[CAT_ORDER] Verify: file content = {}", verifyContent.toString());
            } else {
                ModernShop.LOGGER.error("[CAT_ORDER] Verify FAILED: file does NOT exist after write!");
            }
        } catch (Exception e) {
    ...
    public static void applyPersistedCategoryOrder() {
        ModernShop.LOGGER.info("[CAT_ORDER] applyPersistedCategoryOrder() called — currentServer={}", currentServer != null ? currentServer.getServerDirectory().getAbsolutePath() : "null");
        if (currentServer == null) {
        ...
        try {
            File configDir = new File(currentServer.getServerDirectory(), "config");
            ModernShop.LOGGER.info("[CAT_ORDER] Config dir = {}", configDir.getAbsolutePath());
            ModernShop.LOGGER.info("[CAT_ORDER] Config dir exists = {}", configDir.exists());
            if (configDir.exists()) {
                String[] files = configDir.list();
                if (files != null) {
                    ModernShop.LOGGER.info("[CAT_ORDER] Files in config dir: {}", String.join(", ", files));
                }
            }
            if (!configDir.exists()) {
                ModernShop.LOGGER.info("[CAT_ORDER] Config dir does not exist, cannot load category order");
                return;
            }
            File orderFile = new File(configDir, "modernshop_category_order.json");
            ModernShop.LOGGER.info("[CAT_ORDER] Order file path = {}", orderFile.getAbsolutePath());
            ModernShop.LOGGER.info("[CAT_ORDER] Order file exists = {}", orderFile.exists());
            if (!orderFile.exists()) {
                ModernShop.LOGGER.info("[CAT_ORDER] No persisted category order file found");
                return;
            }
            ...
            ModernShop.LOGGER.info("[CAT_ORDER] Raw file content: '{}'", content.toString());
            ...
            if (nameOrder == null || nameOrder.isEmpty()) {
                ModernShop.LOGGER.info("[CAT_ORDER] Parsed nameOrder is null or empty, not applying");
                return;
            }

            ModernShop.LOGGER.info("[CAT_ORDER] Loaded {} category names from file: {}", nameOrder.size(), nameOrder);
            ...
            ModernShop.LOGGER.info("[CAT_ORDER] Current categories before reorder ({}): {}", categories.size(), currentNames);
            ...
            for (String name : nameOrder) {
                ...
                for (ShopCategory cat : categories) {
                    if (cat.getName().equals(name) && !reordered.contains(cat)) {
                        reordered.add(cat);
                        found = true;
                        ModernShop.LOGGER.info("[CAT_ORDER]   Matched name '{}' -> category '{}'", name, cat.getName());
                        break;
                    }
                }
                if (!found) {
                    ModernShop.LOGGER.warn("[CAT_ORDER]   Name '{}' did NOT match any existing category!", name);
                }
            }
            ...
            for (ShopCategory cat : categories) {
                if (!reordered.contains(cat)) {
                    reordered.add(cat);
                    ModernShop.LOGGER.info("[CAT_ORDER]   Added new category not in saved order: {}", cat.getName());
                }
            }
            if (reordered.size() == categories.size()) {
                categories.clear();
                categories.addAll(reordered);
                ModernShop.LOGGER.info("[CAT_ORDER] SUCCESS: Applied persisted category order ({} categories)", categories.size());
                // Log final order
                ...
                ModernShop.LOGGER.info("[CAT_ORDER] Final category order: [{}]", orderStr.toString());
            } else {
                ModernShop.LOGGER.warn("[CAT_ORDER] Category count mismatch: saved {} names, but have {} categories. Not applying order.",
