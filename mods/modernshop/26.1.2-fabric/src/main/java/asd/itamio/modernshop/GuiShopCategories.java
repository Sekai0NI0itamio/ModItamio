...
public class GuiShopCategories extends Screen {
    ...
    private void saveLayout() {
        ...
        ClientPlayNetworking.send(ShopPacket.reorderCategories(newOrderNames));
    ...
    private void drawLayoutEditor(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        for (int i = startSlot; i < endSlot; i++) {
            ...
            if (catIndex >= 0 && catIndex < categories.size()) {
                ...
                if (!isPickedUp && isMouseInSlot(mouseX, mouseY, x, y, ICON_SIZE, ICON_SIZE)) {
                    ...
                    tooltipList.add(Component.literal(tooltipText));
        ...
        guiGraphics.centeredText(this.font, "\u00a77Page " + (this.scrollOffset + 1) + "/" + maxPages + " " + (rowsPerPage > 0 ? "(Rows " + (this.scrollOffset * rowsPerPage + 1) + "-" + ((this.scrollOffset + rowsPerPage) * rowsPerPage) + ")" : ""), this.width / 2, this.height - 38, 0xAAAAAA);
        guiGraphics.centeredText(this.font, "\u00a7aSave \u00a77\u00a7cCancel", this.width / 2, this.height - 12, 0xAAAAAA);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (layoutEditMode) {
            return handleLayoutClick(event, doubleClick);
        }
        ...
        if (detailView) {
            handleDetailViewClick(event, doubleClick);
            return true;
        }
    ...
    private boolean handleLayoutClick(MouseButtonEvent event, boolean doubleClick) {
        double mouseX = event.x();
        double mouseY = event.y();
        for (var widget : this.children()) {
            if (widget instanceof Button btn && btn.mouseClicked(event, doubleClick)) {
    ...
    private void initDetailView() {
        ...
        this.addRenderableWidget(Button.builder(Component.literal("\u00a7aBuy"), button -> doBuyFromDetail())
    ...
    private void handleDetailViewClick(MouseButtonEvent event, boolean doubleClick) {
        for (var widget : this.children()) {
    ...
    private void sendBuyPacket(int categoryIndex, int itemIndex, int quantity) {
        ClientPlayNetworking.send(ShopPacket.buyItem(categoryIndex, itemIndex, quantity));
    }

    private void sendRemoveCategory(int categoryIndex) {
        ClientPlayNetworking.send(ShopPacket.removeCategory(categoryIndex));
