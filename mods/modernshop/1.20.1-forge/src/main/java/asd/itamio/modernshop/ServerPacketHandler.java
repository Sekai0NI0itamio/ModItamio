...
public class ServerPacketHandler {
    ...
    private static void handleReorderCategories(ServerPlayer player, String[] categoryNames) {
        try {
            ...
            // Persist the new order using ShopData
    ...
    /**
     * Reset the category order to the default (alphabetical or original creation order).
     * OP-only (level 2).
     */
    private static void handleResetCategoryOrder(ServerPlayer player) {
        try {
            ...
            // Clear the persisted category order so defaults are used
    ...
    /**
     * Reset all item price calculations — clear the price config cache.
     * OP-only (level 2).
     */
    ...
    /**
     * Recalculate prices for all items in a specific category.
     * OP-only (level 2).
     */
    ...
    /**
     * Reset a category (remove custom category data, restore default items).
     * OP-only (level 2).
     */
    private static void handleResetCategory(ServerPlayer player, int categoryIndex) {
        try {
            ...
            ShopCategory category = categories.get(categoryIndex);
            // Remove any custom items and reset to default by clearing and re-building
            // For creative tab categories, we keep the items as-is but reset prices
            PriceConfig priceConfig = ModernShop.getPriceEngine().getPriceConfig();
            if (priceConfig != null) {
                for (ItemStack item : category.getItems()) {
                    if (item != null && !item.isEmpty()) {
    ...
    /**
     * Recalculate a single block's value (clear its cached price).
     * OP-only (level 2).
     */
    ...
    /**
     * Reset a single block's value to its default (clear cached price and any overrides).
     * OP-only (level 2).
     */
    private static void handleResetBlock(ServerPlayer player, String itemId) {
        try {
            ...
            if (priceConfig != null) {
                priceConfig.removePrice(stack);
                // Also remove any overrides from ShopData
                MinecraftServer server = ModernShop.getCurrentServer();
                if (server != null) {
                    ShopData shopData = ShopData.forServer(server);
                    // By clearing the price config entry, the override is removed
                }
    ...
    /**
     * Save mod config settings from the client.
     * OP-only (level 2).
     */
